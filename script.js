...
// Auto-play music when user interacts (first click/touch or after DOM loaded)
function autoPlayMusic() {
    // Try playing on page load (may be blocked by some browsers until user gesture)
    bgMusic.volume = 0.45;
    bgMusic.play().catch(() => {
        // If blocked, play after any user gesture
        document.body.addEventListener("pointerdown", handleFirstGesture, { once: true });
    });
}
...
// On CTA button click, start music and scroll (keep this for extra assurance)
function playMusic() {
    bgMusic.play();
    musicIcon.textContent = "🔊";
    musicIcon.classList.remove("paused");
